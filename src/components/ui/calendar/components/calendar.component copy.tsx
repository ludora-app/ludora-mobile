import type { CalendarActiveDateRange, CalendarOnDayPress } from '@marceloterreiro/flash-calendar';
...
import { WindowsXpButton, WindowsXpCalendar, WindowsXpWindow, windowsXpTokens } from './WindowsXpCalendar';
...
export const WindowsXP = () => {
  ...
  const [selectedDate, setSelectedDate] = useState<Date>(sub(new Date(), { days: 1 }));
    ...
    <View style={styles.windowsXpBackground}>
      <WindowsXpWindow title="@marceloterreiro/flash-calendar">
        <Calendar.VStack justifyContent="flex-start" spacing={12}>
          <Text>
            This is a Windows's XP themed calendar, using the composable API pattern to fully customize the calendar's
            appearance.
          </Text>

          <WindowsXpButton onPress={handleOpenPicker}>{format('dd/MM/yyyy')(selectedDate)}</WindowsXpButton>
