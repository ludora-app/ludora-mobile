...
export default function HeaderScreen(props: PropsWithChildren<HeaderProps>) {
  const { children, className, hasNewSession = false, iconProps, isTitleLoading, subTitle, title } = props;

  return (
    <Box className={cn('h-48 flex-row items-end overflow-hidden', { 'h-52': hasNewSession }, className)}>
