...
export default function ProfilEditBioFormsheet() {
  ...
  const onSubmit = async (data: z.infer<typeof schema>) => {
    try {
      ...
      trackEvent({ data: { is_bio_added: !!isBioAdded, is_bio_removed: !!isBioRemoved, is_bio_updated: !!isBioUpdated }, eventName: "profil_edit_bio_success" })
      router.back()
    } catch (error) {
      trackError({ error })
      trackEvent({ data: { error_message: error.message }, eventName: "profil_edit_bio_failed" })
    }
  }
