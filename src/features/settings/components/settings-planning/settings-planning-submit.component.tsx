...
export default function SettingsPlanningSubmit(props: SettingsPlanningSubmitProps) {
  ...
  const { isPending: putUserHoursPreferencesIsPending, mutateAsync: putUserHoursPreferences } = usePutUserHoursPreferences()
  const { isPending: removeUserHoursPreferencesIsPending, mutateAsync: removeUserHoursPreferences } = useRemoveUserHoursPreferences()
  const isDirty = useSettingsPlanningStore(state =>
    checkIsPlanningDirty(initialPlanning, state.planning)
  )
