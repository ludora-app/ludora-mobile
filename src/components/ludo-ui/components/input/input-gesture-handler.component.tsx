...
import { Input as InputChillUi, InputContainer, InputField, InputLabel, cn, Box, BoxRow, IconGestureHandler } from '@chillui/ui';
...
const InputGestureHandler = forwardRef<any, InputProps>((props, ref) => {
  const {
    className,
    error,
    hasClearIcon = true,
  ...
  return (
    <InputChillUi className={className}>
      ...
      >
        {leftIconAction?.name && <IconGestureHandler {...leftIconAction} className="mr-2" />}
        ...
        {rightIconAction?.name && <IconGestureHandler {...rightIconAction} />}
        {showClearIcon && (
          <IconGestureHandler
      ...
      <BoxRow className="items-center gap-2">
        <Box className="flex-1 flex-row items-center">
          {showMessageError && (
            <>
              <IconGestureHandler name="warning-solid" size="xs" color="red" />
...
InputGestureHandler.displayName = "InputGestureHandler"

export default InputGestureHandler;
