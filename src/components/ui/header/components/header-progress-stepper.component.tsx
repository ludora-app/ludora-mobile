...
export default function HeaderProgressStepper({ activeStep = 1, numberOfSteps = 5 }: HeaderProgressStepperProps) {
  return (
    <Box className="relative z-50 flex-row items-center justify-center pb-2">
      {list(1, numberOfSteps).map((step, index) => (
        <React.Fragment key={step}>
          <StepCircle step={step} activeStep={activeStep} />
          {index < numberOfSteps - 1 && <StepConnector step={step} activeStep={activeStep} />}
        </React.Fragment>
      ))}
    </Box>
  );
