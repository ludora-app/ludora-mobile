...
export default function SettingsPreferencesSubmit() {
  ...
  const { isPending: isPutUserSportPreferencesLoading, mutateAsync: putUserSportPreferences } = usePutUserSportPreferences()
  const { isPending: isRemoveUserSportPreferencesLoading, mutateAsync: removeUserSportPreferences } = useRemoveUserSportPreferences()
    ...
    try {
      ...
      trackEvent({
        data: {
          ...
          is_sport_preference_removed: userSportPreferences?.length > 0 && currentSportPreferences.length === 0,
          is_sport_preference_updated: userSportPreferences?.length > 0 && currentSportPreferences.length > 0
        },
